...
def commandMotor(x,y):
    if (scanconfig.cte_verbose):
      print ("Scan step X: " + str(x) + " Y: " + str(y))
...
def dbinsert(dbc, curStep, stepX, stepY,stepXcoord,stepYcoord,mx,my,mcomp,mx_fdback,my_fdback,mcomp_fdback,timestr,dtinit,dtcam):
    item = [curStep,stepX,stepY,stepXcoord,stepYcoord,mx,my,mcomp,mx_fdback,my_fdback,mcomp_fdback,timestr,
            scan_ex_id,dtinit,dtcam,dtcam,dtcam]
...
while (done!=-1 and curStep < endStep ):
    ...
    if (done!=-1):
        ...
        if scanconfig.cte_use_cvcam:
            ret, frame = cam.read()
            #save to disk
            strg=scanconfig.cte_fileprefix+'%s_%03d_%03d.png' % (timestr, scan_ex_id, curStep)
            cv2.imwrite(scanconfig.cte_framePath + strg, frame)
            #show the image
            cv2.imshow('Current Frame', frame)
        if scanconfig.cte_use_photocam:
            # We configure the image capture
            strg='D%s_%03d_%03d.jpg' % (timestr, scan_ex_id, curStep)
            ...
            subprocess.check_output([cmd,args])
        if scanconfig.cte_use_gphoto2:
            strg=scanconfig.cte_gphoto2_filename_root+'%s_%03d_%03d.jpg' % (timestr, scan_ex_id, curStep)
        ...
        #r = requests.post("http://localhost:3000/scan_eng_runs/1/scan_ex_logs/new", data={'scan_ex_log': {"scan_eng_run_id":"1", "a":"6", "b":"8", "c":"9"},"scan_eng_run_id":"1" })
